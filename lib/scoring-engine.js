...
import { setupTokenHistory, getAndUpdateTokenHistory } from './token-history.js';
...
export async function fetchDex(address) {
  try {
    ...
    return {
      ...
      lpLocked:       pair.liquidity?.locked === true  ? true
                        : pair.liquidity?.locked === false ? false
                        : null,
...
export async function secSol(address) {
  try {
    ...
    const lpLocked    = lpHolders.length ? lpHolders.some(lp => lp.is_locked === 1 || lp.tag === 'burn') : null;
...
export async function secEVM(address) {
  try {
    ...
    return {
      ...
      lpLocked:             sec.lp_holder_analysis ? sec.lp_holder_analysis.some(lp => lp.is_locked === 1) : null,
...
// ── BRUTAL Score engine ────────────────────────────────────────────────────
export function calculateScore({ dex: d, sec: s, history = null }) {
  let score = 100;
  const bd  = [];
  ...
  const chg6h = parseFloat(d?.priceChange6h)  || 0;
  const chg1h = parseFloat(d?.priceChange1h)  || 0;
  ...
  // Sustained decline across EVERY timeframe (not just a single bad window)
  // is a live, ongoing dump — not a one-off dip that already stabilized
  if (chg1h < -1 && chg6h < -15 && chg < -25) {
    score -= 15;
    bd.push({ l: `Sustained Dump: down across 1h/6h/24h (${chg1h.toFixed(0)}%/${chg6h.toFixed(0)}%/${chg.toFixed(0)}%)`, p: -15 });
  }
  ...
  if (isActivelyTrading) {
    ...
    if (s?.lpLocked === true)  { score += 6;  bd.push({ l: 'Liquidity Locked', p: +6 }); }
    if (s?.lpLocked === false) { score -= 10; bd.push({ l: 'Liquidity NOT Locked — dev can pull it any time', p: -10 }); }
  ...
  // Already crashed 80%+ in 24h — this is a live rug/dump in progress,
  // clean current stats elsewhere don't undo that
  if (chg < -80) score = Math.min(score, 30);
  else if (chg < -50) score = Math.min(score, 50);
  else if (chg < -30) score = Math.min(score, 70);
  // Sustained decline across every timeframe (1h AND 6h AND 24h all red) —
  // an active, ongoing dump, not a one-off dip. Worse if liquidity isn't
  // even locked, since there's nothing stopping it from going to zero.
  if (chg1h < -1 && chg6h < -15 && chg < -25) {
    score = Math.min(score, s?.lpLocked === false ? 55 : 65);
  }
  // Down big from the highest mcap TrenchReads has ever recorded for this
  // token — catches crashes that have already aged out of the 24h window
  if (history && !history.isFirstScan && history.athMcap > 0 && mcap > 0) {
    const dropFromAth = 1 - (mcap / history.athMcap);
    if (dropFromAth >= 0.8)      score = Math.min(score, 30);
    else if (dropFromAth >= 0.5) score = Math.min(score, 55);
  }
  // Extreme wallet concentration (top 10 hold 80%+) is a near-certain
  // exit-scam setup regardless of how clean everything else looks
  if (top10 !== null && top10 > 80) score = Math.min(score, 40);
  else if (top10 !== null && top10 > 60) score = Math.min(score, 60);
...
export async function runVerdict(address) {
  ...
  // Record this scan and get the token's all-time-high mcap as TrenchReads
  // has observed it — lets the score reflect a crash even after it ages
  // out of DexScreener's rolling 24h window
  let history = null;
  try {
    await setupTokenHistory();
    history = await getAndUpdateTokenHistory(address, chain, d?.marketCap || 0);
  } catch (e) {
    console.error('token-history lookup failed (non-fatal):', e.message);
  }

  const scoreData = calculateScore({ dex: d, sec: s, history });
