/**
 * TrenchReads Scoring Engine
 * Single source of truth — used by:
 *   - api/check.js        (web frontend)
 *   - telegram-bot.js     (via api/check.js HTTP call)
 *   - api/v1/verdict.js   (B2B API)
 *
 * Exports: runVerdict(address) → full result object
 */
...
export async function secSol(address) {
  try {
    ...
    return {
      ...
      top10HolderPct:       sec.top10_holder_ratio  ? (parseFloat(sec.top10_holder_ratio) * 100).toFixed(1) : null,
      topHolders,
...
export async function secEVM(address) {
  try {
    ...
    return {
      ...
      holderCount:          sec.holder_count         || null,
      top10HolderPct:       sec.top10_holder_percent ? (parseFloat(sec.top10_holder_percent) * 100).toFixed(1) : null,
      topHolders,
      creatorPct:           sec.creator_percent      ? (parseFloat(sec.creator_percent) * 100).toFixed(2) : null,
      creatorAddress:       sec.creator_address      || null,
      isProxy:              sec.is_proxy             === '1',
...
export function calculateScore({ dex: d, sec: s }) {
  ...
  if (s?.isHoneypot)  { score -= 40; bd.push({ l: 'Honeypot Detected',     p: -40 }); }
  ...
  if (s?.canTakeBackOwnership || s?.hiddenOwner) { score -= 15; bd.push({ l: 'Owner Control Risk',  p: -15 }); }
  if (s?.transferPausable)  { score -= 10; bd.push({ l: 'Transfers Pausable',    p: -10 }); }
  if (s?.isBlacklisted)     { score -= 8;  bd.push({ l: 'Blacklist Function',    p: -8  }); }
  if (s?.selfDestruct)      { score -= 10; bd.push({ l: 'Self-Destruct Fn',      p: -10 }); }

  const liq  = parseFloat(d?.liquidity)  || 0;
  const mcap = parseFloat(d?.marketCap)  || 0;
  const vol  = parseFloat(d?.volume24h)  || 0;

  if (liq === 0)           { score -= 30; bd.push({ l: 'No Liquidity',              p: -30 }); }
  else if (liq < 2000)     { score -= 20; bd.push({ l: `Critical Low Liq $${fmt(liq)}`, p: -20 }); }
  else if (liq < 10000)    { score -= 10; bd.push({ l: `Low Liquidity $${fmt(liq)}`,    p: -10 }); }

  if (liq > 0 && mcap > 0) {
    const rugRatio = (liq / mcap) * 100;
    if (rugRatio < 0.5)      { score -= 25; bd.push({ l: `Rug Exit Risk ${rugRatio.toFixed(2)}% Liq/MCap`,    p: -25 }); }
    else if (rugRatio < 1)   { score -= 18; bd.push({ l: `Very Low Liq/MCap ${rugRatio.toFixed(2)}%`,         p: -18 }); }
    else if (rugRatio < 3)   { score -= 10; bd.push({ l: `Low Rug Buffer ${rugRatio.toFixed(1)}% Liq/MCap`,   p: -10 }); }
  }

  const buys  = d?.txns24h?.buys  || 0;
  const sells = d?.txns24h?.sells || 0;
  if (buys === 0 && sells === 0 && liq > 0) { score -= 20; bd.push({ l: 'Dead Token — Zero Activity', p: -20 }); }
  else if (vol < 500 && liq > 0)            { score -= 8;  bd.push({ l: 'Very Low Volume',             p: -8  }); }
  ...
  if (s?.lpLocked) { score += 5; bd.push({ l: 'Liquidity Locked', p: +5 }); }
