...
export function calculateScore({ dex: d, sec: s }) {
  ...
  // ── LIQ/MCAP ratio — primary rug signal ───────────────────────────────
  if (liq > 0 && mcap > 0) {
    const rugRatio = (liq / mcap) * 100;
    if (rugRatio < 0.5)      { score -= 28; bd.push({ l: `Rug Exit Risk ${rugRatio.toFixed(2)}% Liq/MCap`, p: -28 }); }
    else if (rugRatio < 1)   { score -= 20; bd.push({ l: `Critical Low Liq/MCap ${rugRatio.toFixed(2)}%`, p: -20 }); }
    else if (rugRatio < 3)   { score -= 12; bd.push({ l: `Low Rug Buffer ${rugRatio.toFixed(1)}% Liq/MCap`, p: -12 }); }
    else if (rugRatio < 5)   { score -= 5;  bd.push({ l: `Thin Liq/MCap ${rugRatio.toFixed(1)}%`, p: -5 }); }
  }
  ...
  if (liq < 1000 && liq > 0) score = Math.min(score, 40);
  // Token with < $5K liquidity can NEVER score above 55
  if (liq < 5000 && liq > 0) score = Math.min(score, 55);
  // Token with < $100 volume can NEVER score above 50
  if (vol < 100 && liq > 0)  score = Math.min(score, 50);
  // Dead token (no buys/sells) can NEVER score above 35
